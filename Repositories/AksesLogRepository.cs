...
public class AksesLogRepository : IAksesLogRepository
{
    ...
    // Helper query agar tidak copy-paste Include berkali-kali 
    private IQueryable<AksesLog> GetBaseQuery()
    {
        return _context.AksesLog
            .Include(a => a.Ruangan)
            .Include(a => a.Kartu)
                .ThenInclude(k => k!.Kelas) // Load Kelas Kartu
            .Include(a => a.Kartu)
                .ThenInclude(k => k!.User)
                    .ThenInclude(u => u!.Kelas) // <--- PENTING: Load Kelas User
            .AsNoTracking();
    }
    ...
    // ... Stats methods tetap sama ...
    public async Task<int> CountByKelasIdAsync(int kelasId) => await _context.AksesLog.Include(a => a.Kartu).CountAsync(a => a.Kartu != null && a.Kartu.KelasId == kelasId);
