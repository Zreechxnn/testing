...
public class AksesLogService : IAksesLogService
{
    ...
    public async Task<ApiResponse<List<MonthlyStatsDto>>> GetLast12MonthsStats()
    {
        try
        {
            // 1. Hitung tanggal mulai dengan benar (12 bulan terakhir termasuk bulan ini)
            var nowUtc = DateTime.UtcNow;
            var todayUtc = nowUtc.Date;

            // Mulai dari tanggal 1 di bulan yang 11 bulan lalu dari bulan ini
            // Contoh: jika sekarang Jan 2026, 12 bulan terakhir = Feb 2025 - Jan 2026
            var startDate = new DateTime(todayUtc.Year, todayUtc.Month, 1).AddMonths(-11);

            // Akhir query: akhir hari ini (sampai jam 23:59:59)
            var endDate = todayUtc.AddDays(1).AddTicks(-1);

            _logger.LogInformation("GetLast12MonthsStats - Query range: {StartDate} to {EndDate}",
            ...
            _logger.LogInformation("GetLast12MonthsStats - Raw data count: {Count}", rawData.Count);
            foreach (var kvp in rawData)
            {
                _logger.LogInformation("Month: {Month}, Count: {Count}",
                    kvp.Key.ToString("yyyy-MM"), kvp.Value);
            }
            ...
            // 4. Generate 12 bulan terakhir termasuk bulan ini
            for (int i = 0; i < 12; i++)
            {
                ...
                var total = rawData.ContainsKey(monthKey) ? rawData[monthKey] : 0;
                ...
                _logger.LogDebug("Month {Label}: {Total}", label, total);
            }
        ...
        {
            _logger.LogError(ex, "Error getting last 12 months stats: {Message}", ex.Message);
            return ApiResponse<List<MonthlyStatsDto>>.ErrorResult($"Gagal mengambil statistik 12 bulan terakhir: {ex.Message}");
        }
    }
